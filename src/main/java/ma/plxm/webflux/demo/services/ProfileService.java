...
import ma.plxm.webflux.demo.events.ProfileCreatedEvent;
...
public class ProfileService {
    ...
    ProfileService(ApplicationEventPublisher publisher,
    ...
    public Mono<Profile> delete(String id) {
        return this.profileRepository.findById(id).flatMap(
            p -> this.profileRepository.deleteById(p.getId()).thenReturn(p));
    }
    ...
    public Mono<Profile> create(String email) {
        return this.profileRepository.save(new Profile(null, email))
            .doOnSuccess(profile -> this.publisher
                .publishEvent(new ProfileCreatedEvent(profile)));
    }
}
