package ma.plxm.webflux.demo.websocket.config;
...
class WebSocketConfigurationTest {
    ...
    public void testNotificationsOnUpdates() throws Exception {
        ...
        socketClient.execute(uri, (WebSocketSession session) -> {
            ...
            return session.send(out).thenMany(in).doOnNext(str -> {
                log.info("receiving " + str);
        ...
        Flux.<Profile>generate(sink -> sink.next(generateRandomProfile()))
            .take(count).flatMap(this::write).blockLast();
    ...
    private Publisher<Profile> write(Profile p) {
        return this.webClient.post().uri("http://localhost:8080/profiles")
            .body(BodyInserters.fromObject(p)).retrieve()
